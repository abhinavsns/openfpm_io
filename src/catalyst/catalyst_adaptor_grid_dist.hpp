...
// Catalyst Adaptor for structured particles mesh, stored in grid_dist-like data structures.
...
class catalyst_adaptor<grid_type, vis_props<props...>, catalyst_adaptor_impl::GRID_DIST>
{
    ...
    // SoA storage for (subset of) properties of particles, split across local sub-grids.
...
public:
    ...
    void execute(grid_type &grid, size_t cycle = 0, double time = 0, const std::string &channel_name = "grid")
    {
        ...
        for (int i = 0; i < grid_type::dims; i++)
        {
            ...
            {
                std::cerr << __FILE__ << ":" << __LINE__ << " Warning: ghost must be bigger than spacing or you will have artifacts on the process boundaries" << std::endl;
            }
        }
        //grid.template map();
